    ...
    do
    {
        ...
        if (!goData)
        {
           // TC_LOG_ERROR("sql.sql", "GameObject (GUID: %u) does not exist but has a record in `gameobject_addon`", guid);
			TC_LOG_ERROR("sql.sql", "DELETE FROM gameobject_addon WHERE guid = %u;", guid);
            continue;
        }
    ...
    do
    {
        ...
        if (!data)
        {
           // TC_LOG_ERROR("server.loading", "Spawn data with ID (%u,%u) not found, but is listed as a member of spawn group %u!", uint32(spawnType), spawnId, groupId);
			if (sConfigMgr->GetBoolDefault("TrinityCoreLog", true));
			TC_LOG_ERROR("server.loading", "DELETE FROM spawn_group WHERE groupId = %u AND spawnType = %u AND spawnId = %u;", groupId, uint32(spawnType), spawnId);
            continue;
        }
...
void ObjectMgr::LoadItemTemplates()
{
    ...
    do
    {
        ...
        for (uint8 i = 0; i < itemTemplate.StatsCount; ++i)
        {
			...
			if (sConfigMgr->GetBoolDefault("ItemStatsMOD", true))
			{
				float modstats = sConfigMgr->GetFloatDefault("StatsValueMulti", 1.0f);
				itemTemplate.ItemStat[i].ItemStatValue = (uint32(fields[29 + i * 2].GetInt32()))*modstats;
			}
			else
			{
        ...
        for (uint8 i = 0; i < MAX_ITEM_PROTO_DAMAGES; ++i)
		{
			if (sConfigMgr->GetBoolDefault("ItemStatsMOD", true))
			{
				float dmgmod = sConfigMgr->GetFloatDefault("ItemDmgMOD", 1.0f);
				itemTemplate.Damage[i].DamageMin = (fields[50 + i * 3].GetFloat())*dmgmod;
				itemTemplate.Damage[i].DamageMax = (fields[51 + i * 3].GetFloat())*dmgmod;
			}
			else
			{
		...
		if (sConfigMgr->GetBoolDefault("ItemStatsMOD", true))
		{
			float armormod = sConfigMgr->GetFloatDefault("ItemArmorMOD", 1.0f);
			itemTemplate.Armor = (uint32(fields[56].GetUInt32()))*armormod;
		}
		else
		...
		{
			int16 a = 255;
			itemTemplate.HolyRes = (uint32(fields[57].GetUInt16()))+a;
			itemTemplate.FireRes = (uint32(fields[58].GetUInt16()))+a;
			itemTemplate.NatureRes = (uint32(fields[59].GetUInt16()))+a;
			itemTemplate.FrostRes = (uint32(fields[60].GetUInt16()))+a;
			itemTemplate.ShadowRes = (uint32(fields[61].GetUInt16()))+a;
			itemTemplate.ArcaneRes = (uint32(fields[62].GetUInt16()))+a;
		}
		}
		itemTemplate.HolyRes = uint32(fields[57].GetUInt16());
		itemTemplate.FireRes = uint32(fields[58].GetUInt16());
		itemTemplate.NatureRes = uint32(fields[59].GetUInt16());
		itemTemplate.FrostRes = uint32(fields[60].GetUInt16());
		itemTemplate.ShadowRes = uint32(fields[61].GetUInt16());
		itemTemplate.ArcaneRes = uint32(fields[62].GetUInt16());
		if (sConfigMgr->GetBoolDefault("ItemDelayMOD", false))
			itemTemplate.Delay = (uint32(fields[63].GetUInt16())) / sConfigMgr->GetIntDefault("ItemDelay", 0);
		else
