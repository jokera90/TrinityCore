...
public:
	...
	static bool HandleModifyMountCommand(ChatHandler* handler, char const* args)
	{
		...
		const char* chmount = strtok((char*)args, " ");
		const char* chspeed = strtok(NULL, " ");
		uint32 mount = atol(chmount);
		...
		float speed;
		if (!CheckModifySpeed(handler, chspeed, target, speed, 0.1f, 50.0f, false))
			return false;
		...
		target->SetSpeedRate(MOVE_RUN,speed+target->GetSpeedRate(MOVE_RUN));
		target->SetSpeedRate(MOVE_FLIGHT, speed +target->GetSpeedRate(MOVE_FLIGHT));
