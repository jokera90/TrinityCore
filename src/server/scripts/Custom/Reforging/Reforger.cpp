...
static const ItemModType statTypes[] = { ITEM_MOD_SPIRIT, ITEM_MOD_DODGE_RATING, ITEM_MOD_PARRY_RATING, ITEM_MOD_HIT_RATING, ITEM_MOD_CRIT_RATING, ITEM_MOD_HASTE_RATING, ITEM_MOD_EXPERTISE_RATING, ITEM_MOD_INTELLECT, ITEM_MOD_STRENGTH, ITEM_MOD_AGILITY, ITEM_MOD_STAMINA, ITEM_MOD_SPELL_POWER, ITEM_MOD_HEALTH_REGEN };
static const uint8 stat_type_max = sizeof(statTypes) / sizeof(*statTypes);
//float tengu = sConfigMgr->GetFloatDefault("A", 1.0f);
...
static const char* GetStatName(uint32 ItemStatType)
{
    switch (ItemStatType)
    {
		...
		case ITEM_MOD_STRENGTH: return "Strenght"; break;
		case ITEM_MOD_AGILITY: return "Agilty"; break;
		...
		case ITEM_MOD_HEALTH_REGEN: return "HP regen"; break;
        default: return NULL;
    }
}
...
static bool IsReforgable(Item* invItem, Player* player)
{
    ...
    for (uint32 i = 0; i < pProto->StatsCount; ++i)
    {
        if (!GetStatName(pProto->ItemStat[i].ItemStatType))
            continue;
        if (((int32)floorf((float)pProto->ItemStat[i].ItemStatValue * sConfigMgr->GetFloatDefault("Modify ReforgetStats", 0.0f))) > 1)
            return true;
    }
    return false;
}
...
static void UpdatePlayerReforgeStats(Item* invItem, Player* player, uint32 decrease, uint32 increase) // stat types
{
    ...
    for (uint32 i = 0; i < pProto->StatsCount; ++i)
    {
        ...
        if (pProto->ItemStat[i].ItemStatType == decrease)
            stat_diff = (int32)floorf((float)pProto->ItemStat[i].ItemStatValue * sConfigMgr->GetFloatDefault("Modify ReforgetStats", 0.0f));
    }
    if (stat_diff <= 0)
            ...
            switch (menu)
            {
                ...
                case SELECT_STAT_REDUCE:
                    ...
                    if (Item* invItem = player->GetItemByPos(INVENTORY_SLOT_BAG_0, action))
                    {
                        if (IsReforgable(invItem, player))
                        {
                            ...
                            for (uint32 i = 0; i < pProto->StatsCount; ++i)
                            {
                                int32 stat_diff = ((int32)floorf((float)pProto->ItemStat[i].ItemStatValue *sConfigMgr->GetFloatDefault("Modify ReforgetStats", 0.0f)));
                                if (stat_diff > 1)
                ...
                case SELECT_STAT_INCREASE:
                    ...
                    // action = StatsCount id
                    {
                        ...
                        {
                            const ItemTemplate* pProto = invItem->GetTemplate();
                            int32 stat_diff = ((int32)floorf((float)pProto->ItemStat[action].ItemStatValue * sConfigMgr->GetFloatDefault("Modify ReforgetStats", 0.0f)));
