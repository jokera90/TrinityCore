...
public:
	...
	class BountyHunter_AI : public ScriptedAI
	{
	public:
		BountyHunter_AI(Creature* creature) :ScriptedAI(creature){}
		...
		static bool OnGossipHello(Player * player, Creature * Creature)
		{
			...
			printf_s("pritnt is ok");
			SendGossipMenuFor(player, 907, Creature->GetGUID());
			return true;
		}
		bool GossipSelect(Player* player, uint32 menuid, uint32 gossipmenuid) override
		{
			uint32 sender = player->PlayerTalkClass->GetGossipOptionAction(gossipmenuid);
		...
		static bool OnGossipSelect(Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
		{
			...
			case GOSSIP_ACTION_INFO_DEF + 1:
			{
				...
				SendGossipMenuFor(pPlayer, 365, pCreature->GetGUID());
				break;
			}
...
#endif
				SendGossipMenuFor(pPlayer, 878, pCreature->GetGUID());
				break;
			}
		...
		bool GossipSelectCode(Player* player, uint32 menuid, uint32 gossipmenuid, const char* code) override
		{
			uint32 sender = player->PlayerTalkClass->GetGossipOptionSender(gossipmenuid);
			uint32 action = player->PlayerTalkClass->GetGossipOptionAction(gossipmenuid);
	...
	{
		return new BountyHunter_AI(creature);
	}
};
...
class BountyKills : public PlayerScript
{
	public:
		...
		{
			QueryResult result = CharacterDatabase.PQuery("SELECT * FROM bounties WHERE guid='%u'", player->GetGUID());
		...
		void OnPVPKill(Player * Killer, Player * Bounty)
		{
			...
			Field * fields = result->Fetch();
				...
				case 1:
				{
					...
					Killer->SetPvP(false);
					Killer->RemoveByteFlag(UNIT_FIELD_BYTES_2, 1, UNIT_BYTE2_FLAG_FFA_PVP);
					break;
				}
				case 2:
