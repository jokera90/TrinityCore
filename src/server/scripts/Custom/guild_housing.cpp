...
void SpawnObject(Player * pPlayer, uint32 objectId)
{
	...
	QuaternionData rot = QuaternionData::fromEulerAnglesZYX(z, 0.0f, 0.0f);
	ObjectGuid::LowType guidLow = map->GenerateLowGuid<HighGuid::GameObject>();

	if (!object->Create(guidLow, objectInfo->entry, map, pPlayer->GetPhaseMaskForSpawn(), *pPlayer, rot, 255, GO_STATE_READY))
	{
		delete object;
		return;
	}
	...
	if (ObjectSpawns)
	{
		do
		{
			...
			if (GameObjectData const* gameObjectData = sObjectMgr->GetGameObjectData(Object_GUID))
				object = ChatHandler(pPlayer->GetSession()).GetObjectFromPlayerMapByDbGuid(Object_GUID);
...
void SellGuildHouse(Player * pPlayer, Creature * pCreature)
{
	if (isInGuildArea(pPlayer))
	{
		...
		pCreature->Say("You have successfully sold your guild house!", LANG_UNIVERSAL, pPlayer);
...
void PurchaseGuildHouse(Player * pPlayer, Creature * pCreature, uint32 &Id)
{
	if (pPlayer->HasItemCount(GUILD_TOKEN, 1))
	{
		...
		pCreature->Say("Congratulations on the purchase of your new guild house!", LANG_UNIVERSAL,pPlayer);
		pPlayer->DestroyItemCount(GUILD_TOKEN, 1, true);
		requiresGUpdate = true;
	}
	else
	{
		pCreature->Say("You need a guild token to purchase a guild house!", LANG_UNIVERSAL, pPlayer);
	}
...
public:
	...
	class GuildHouse_NPCAI : public ScriptedAI
	{
	public:
		GuildHouse_NPCAI(Creature* pCreature) : ScriptedAI(pCreature){}
	...
	void ShowHouseListings(Player * pPlayer, Creature * pCreature, unsigned int start = 0)
	{
		...
		for (unsigned int i = start; i < houses; i++)
		{
			...
			if (offset != 10)
			{
				AddGossipItemFor(pPlayer,6, pField[1].GetString(), GOSSIP_SENDER_MAIN, pField[0].GetUInt32());
				offset++;
				pResult->NextRow();
			}
			else
			{
				...
				{
					AddGossipItemFor(pPlayer,7, "-------------END--------------", GOSSIP_SENDER_MAIN, 41);
				}
				else
				{
					AddGossipItemFor(pPlayer,7, "Next Page", GOSSIP_SENDER_MAIN, 4353);
	...
	bool OnGossipHello(Player * pPlayer, Creature * pCreature)
	{
		...
		AddGossipItemFor(pPlayer,4, "Guild Housing System:", GOSSIP_SENDER_MAIN, 41);
		AddGossipItemFor(pPlayer,4, "----------------------------------------------", GOSSIP_SENDER_MAIN, 41);
		if (isPlayerGuildLeader(pPlayer))
		{
			if (!GuildOwnsHouse(pPlayer))
				AddGossipItemFor(pPlayer,6, "Buy a Guild House", GOSSIP_SENDER_MAIN, 38);
			else
			AddGossipItemFor(pPlayer,6, "Sell Guild House", GOSSIP_SENDER_MAIN, 39, "Are you sure you want to sell your guild house?", 0, false);
		}
		if (GuildOwnsHouse(pPlayer))
			AddGossipItemFor(pPlayer,7, "Teleport to Guild House", GOSSIP_SENDER_MAIN, 40);
		pPlayer->PlayerTalkClass->SendGossipMenu(2, pCreature->GetGUID());
		return true;
	}
	bool GossipSelect(Player* pPlayer, uint32 menuid, uint32 gossipListId)
	{
...
{
	return new GuildHouse_NPCAI(pCreature);
}
};
...
public:
	...
	class Guild_Token_VendorAI : public ScriptedAI
	{
public:
	Guild_Token_VendorAI(Creature*pCreature) : ScriptedAI(pCreature){}

	bool HelloGossip(Player*pPlayer)
	{
		return OnGossipHello(pPlayer, me);
	}
	bool OnGossipHello(Player * pPlayer, Creature * pCreature)
	{
		...
		{
			pPlayer->PlayerTalkClass->ClearMenus();
			AddGossipItemFor(pPlayer,4, "I would like to purchase Object Tokens!", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_TRADE);
			AddGossipItemFor(pPlayer,3, "Nevermind.", GOSSIP_SENDER_MAIN, 2);
			pPlayer->PlayerTalkClass->SendGossipMenu(2, pCreature->GetGUID());
			//return true;
		}
		else
	...
	CreatureAI*GetAI(Creature*pCreature) const override
	{
		return new Guild_Token_VendorAI(pCreature);
	}
};
